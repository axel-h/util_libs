...
int uart_putchar(ps_chardevice_t* dev, int c)
{
    ...
    /* Check if the TX FIFO has space. If not, then fail if SERIAL_NONBLOCKING
     * is set and do busy waiting otherwise.
     */
    if (internal_uart_is_tx_fifo_full(regs))
        if (d->flags & SERIAL_NONBLOCKING) {
            return -1;
        }
     ...
     * thing to do for a serial terminal. Since we want to make CR/LR atomic, a
     * blocking wait will be used to ensure the LF can be sent also.
     */
